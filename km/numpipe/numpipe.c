...
static int max_size = 0; // maximum size of queue 
...
static ssize_t device_read(struct file* filep, char* buffer, size_t length, loff_t* offset) {
	...
	down_interruptible(&full_slots_n); // decreases the # of full slots ; if already 0, user-process blocks
	mutex_lock_interruptible(&lock); // obtains the lock if available
	item = queue[head_ptr];
...
static ssize_t device_write(struct file* filep, const char* buffer, size_t length, loff_t* offset) {
	...
	down_interruptible(&empty_slots_n); // decrease the number of empty slots
	mutex_lock_interruptible(&lock); // obtain lock
